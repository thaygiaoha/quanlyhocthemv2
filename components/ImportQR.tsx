...
const ImportSection: React.FC<ImportSectionProps> = ({ data, onUpdate }) => {
  ...
  const [manualStudent, setManualStudent] = useState({
    name: 'Nguyễn Văn Hà Pro', class: '12A1', school: 'THPT Yên Dũng số 2', phoneNumber: '0988948882', note: '', code: 'HS1201' 
  });
const handleAuth = async () => {
 ...
 const handleClearData = () => {
  ...
  const downloadTemplate = () => {
    const templateData = [
      { "STT": 1, "Họ và Tên": "Nguyễn Văn Hà Pro", "Lớp": "12A1", "Trường": "THPT Yên Dũng số 2", "Số điện thoại": "0988948882", "Lớp phụ": '', "Mã HS": 'HS1201' }
    ];
  ...
  const processExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
    // 2207them3: Kiểm tra bản quyền VIP khi nhập danh sách học sinh từ Excel
    if (data.enableCopyrightCheck !== false && data.licenseStatus !== 'vip') {
      alert("Tính năng nhập danh sách học sinh chỉ dành cho tài khoản VIP. Vui lòng đăng ký/kích hoạt bản quyền VIP để sử dụng!");
      e.target.value = '';
      return;
    }
...
const handleManualAdd = async () => {
  // 2207them3: Kiểm tra bản quyền VIP khi thêm học sinh thủ công
  if (data.enableCopyrightCheck !== false && data.licenseStatus !== 'vip') {
    alert("Tính năng nhập/thêm học sinh chỉ dành cho tài khoản VIP. Vui lòng đăng ký/kích hoạt bản quyền VIP để sử dụng!");
    return;
  }

  setManualStudent({ name: 'Nguyễn Văn Hà Pro', class: '12A1', school: 'THPT Yên Dũng số 2', phoneNumber: '0988948882', note: '', code: 'HS1201' });
