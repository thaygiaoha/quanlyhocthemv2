...
const ImportQR: React.FC<ImportQRProps> = ({ data, onUpdate }) => {
  // Sửa lần 2: Sử dụng sessionStorage để giữ trạng thái đăng nhập đồng bộ giữa các tab
  ...
  try {
    ...
    if (key === "16868688") {        
      ...
      alert('Sai mật khẩu rồi nhé bạn.');
    }   
  } catch (error) {
    ...
    reader.onload = (evt) => {
      try {
        ...
        jsonData.slice(1).forEach((row: any, index: number) => {
          ...
          const stt = parseInt(row['A']) || (index + 1);
          const name = String(row['B'] || '').trim();
          const className = String(row['C'] || '').trim();
          const amount = parseFloat(row['D']) || 0;
          const bankId = String(row['E'] || '').trim();
          const accountNo = String(row['F'] || '').trim();
          const bankName = String(row['G'] || '').trim();
          const accountName = String(row['H'] || '').trim();
          // Lấy giá trị cột I, xóa khoảng trắng thừa
          const rawI = String(row['I'] || '').trim();

          // Luôn đảm bảo bắt đầu bằng "SEVQR", nếu cột I có chữ thì ghép thêm vào
          const content = rawI ? `SEVQR ${rawI}` : '';
          ...
          parsedItems.push({
            ...
            className,
            amount,
            bankId: bankId || 'Vietinbank',
            accountNo: accountNo || '104887594225',
            bankName: bankName || 'Vietinbank',
            accountName: accountName || 'HKD HA THAO',
            content: content || ""
          });
        });
        ...
        alert("Lỗi phân tích file Excel. Vui lòng tải đúng file mẫu chuẩn!");
        console.error(err);
      }
    };
